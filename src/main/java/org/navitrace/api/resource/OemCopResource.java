...
import org.navitrace.model.Oemcop;
...
public class OemCopResource extends BaseResource {

    @Path("{id}")
    @GET
    public Response getSingle(@PathParam("id") long id) throws StorageException {
        Oemcop entity = storage.getObject(Oemcop.class, new Request(
    ...
    public Response getAll(@QueryParam("pageNo") int pageNo,
        ...
        Collection<Oemcop> oemcops = storage.getObjects(Oemcop.class,
                new Request(new Columns.All(), new Order(sortField, desc, pageNo, pageSize)));
        return Response.ok(oemcops).build();
    }

    @POST
    public Response add(Oemcop entity) throws Exception {
    ...
    @Path("{id}")
    @PUT
    public Response update(Oemcop entity) throws Exception {
        storage.updateObject(Oemcop.class, new Request(
    ...
    public Response remove(@PathParam("id") long id) throws Exception {
        storage.removeObject(Oemcop.class, new Request(new Condition.Equals("id", id)));
