...
import org.navitrace.model.Rto;
...
public class RtoResource extends BaseResource {

    @Path("{id}")
    @GET
    public Response getSingle(@PathParam("id") long id) throws StorageException {
        Rto entity = storage.getObject(Rto.class, new Request(
    ...
    public Response getAll(@QueryParam("pageNo") int pageNo,
        ...
        Collection<Rto> rtos = storage.getObjects(Rto.class,
                new Request(new Columns.All(), new Order(sortField, desc, pageNo, pageSize)));
        return Response.ok(rtos).build();
    }

    @POST
    public Response add(Rto entity) throws Exception {
    ...
    @Path("{id}")
    @PUT
    public Response update(Rto entity) throws Exception {
        storage.updateObject(Rto.class, new Request(
    ...
    public Response remove(@PathParam("id") long id) throws Exception {
        storage.removeObject(Rto.class, new Request(new Condition.Equals("id", id)));
