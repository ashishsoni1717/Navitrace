...
import org.navitrace.model.Vmake;
...
public class VehicleMakeResource extends BaseResource {

    @Path("{id}")
    @GET
    public Response getSingle(@PathParam("id") long id) throws StorageException {
        Vmake entity = storage.getObject(Vmake.class, new Request(
    ...
    public Response getAll(@QueryParam("pageNo") int pageNo,
        ...
        Collection<Vmake> vmakes = storage.getObjects(Vmake.class,
                new Request(new Columns.All(), new Order(sortField, desc, pageNo, pageSize)));
        return Response.ok(vmakes).build();
    }

    @POST
    public Response add(Vmake entity) throws Exception {
    ...
    @Path("{id}")
    @PUT
    public Response update(Vmake entity) throws Exception {
        storage.updateObject(Vmake.class, new Request(
    ...
    public Response remove(@PathParam("id") long id) throws Exception {
        storage.removeObject(Vmake.class, new Request(new Condition.Equals("id", id)));
